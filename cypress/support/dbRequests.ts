export class SQLQueries {
    ...
    static checkIfBodyMessageSent(email: string, message: string,via: string) {
        const envVars:Object = Cypress.env("db")
        const query = `select message_plain from sendajobprod.accounts as acc join sendajobprod.messages_log as mes on acc.id = mes.account_id where email_address = '${email}' and via = '${via}' order by timestamp desc limit 1;`;
